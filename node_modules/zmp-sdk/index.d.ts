import { AsyncVoidCallback as AsyncVoidCallback$1, StreamType as StreamType$1, MediaConstraints as MediaConstraints$1, PhotoConstraint as PhotoConstraint$1, PhotoFrame as PhotoFrame$1, MediaDevice as MediaDevice$1, CameraEvents as CameraEvents$1 } from '@mp-types';
...
declare const openDocumentArgs: ZodObject<{
    url: ZodString;
    title: ZodOptional<ZodString>;
    download: ZodOptional<ZodDefault<ZodBoolean>>;
    edit: ZodOptional<ZodDefault<ZodBoolean>>;
}, "strip", ZodTypeAny, {
    title?: string | undefined;
...
/**
 * API cho phép ứng dụng xem trực tiếp tài liệu (PDF) từ URL.
 *
 * @minimumVersion 2.47.0
 * @zaloOnly Chỉ hoạt động khi chạy bên trong ứng dụng Zalo mobile.
 * @param {string} url URL của tài liệu cần mở. Hiện chỉ hỗ trợ định dạng tài liệu PDF.
 * @param {string} [title] Tiêu đề của tài liệu trên header bar. Mặc định lấy tên từ URL.
 * @param {boolean} [download=false] Bật tính năng tải tài liệu. Mặc định là false.
 * @param {boolean} [edit=false] Bật tính năng chỉnh sửa tài liệu (chỉ hỗ trợ trên ios). Mặc định là false.
 *
 * @example Mở tài liệu PDF từ URL:
 * ```ts
 * openDocument({
 ...
 * ```
 *
 * @example Mở tài liệu PDF từ URL với tiêu đề tùy chỉnh:
 * ```ts
 * openDocument({
 *   url: "https://pdfobject.com/pdf/sample.pdf",
 *   title: "Tài liệu mẫu"
 ...
 * ```
 *
 * @example Mở tài liệu PDF từ URL với tiêu đề tùy chỉnh và mở tính năng tải xuống:
 * ```ts
 * openDocument({
 *   url: "https://pdfobject.com/pdf/sample.pdf",
 *   title: "Tài liệu mẫu",
 ...
 * ```
 *
 * @example Mở tài liệu PDF từ URL với tiêu đề tùy chỉnh và mở tính năng chỉnh sửa (chỉ hỗ trợ trên ios):
 * ```ts
 * openDocument({
 *   url: "https://pdfobject.com/pdf/sample.pdf",
 *   title: "Tài liệu mẫu",
 ...
 * ```
 */
declare function openDocument(args: AsyncVoidCallback & TypeOf<(typeof openDocumentArgs)[0]>): Promise<void>;

/**
 * Lấy thông tin nguồn mở app.
 *
 * @minimumVersion 2.48.1
 * @zaloOnly Chỉ hoạt động khi chạy bên trong ứng dụng Zalo mobile.
 *
 * @returns {Object} Trả về đối tượng chứa thông tin nguồn mở app.
 * - `srcOpen`: Nguồn mở app.
 *
 * @example Lấy thông tin nguồn mở app:
 * ```ts
 * const entrySource = await getEntrySource();
 * console.log(entrySource.srcOpen);
 * ```
 *
 * @example Lấy thông tin nguồn mở app với callback:
 * ```ts
 * getEntrySource({
 *   success: function(res) {
 *     console.log(res.srcOpen);
 *   },
 *   fail: function(err) {
 *     console.error(err);
 *   }
 ...
 * ```
 */
declare function getEntrySource(args?: AsyncCallback<{
    srcOpen: string;
}>): Promise<{
    srcOpen: string;
} | null>;

declare function getDeviceInfo(args?: AsyncCallback<GetDeviceInfoReturn>): Promise<GetDeviceInfoReturn>;
...
declare const otherApis: {
    ...
    showCustomOAWidget: typeof showCustomOAWidget;
    CheckoutKit: {
        showWidget: typeof showWidget;
        purchase: typeof purchase$1;
    };
    openDocument: typeof openDocument;
    getEntrySource: typeof getEntrySource;
